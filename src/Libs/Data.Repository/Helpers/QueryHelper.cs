...
namespace Data.Repository.Helpers
{
    public static class QueryHelper
    {
        ...
        private static async Task<PagedList<T>> SearchPageAsync<T>(this IQueryable<T> source,
            Paging paging, IFilter filter, SortBase<T> sort, FilterOperator? filterOperator, SelectBase select, Func<List<T>, Task> apply = null)
        {
            if (sort != null)
            {
            ...
            if (select != null)
            {
                source = SelectHelper.ApplySelect(source, select);
            }
